...
export class FetchResponseTypePlugin implements ExecutorPlugin {
  ...
  async onSuccess(
    ...
    console.log('FetchResponseTypePlugin onSuccess', context,config.responseType);

    try {
      switch (config.responseType) {
        case 'json':
          const json = await response.json();
        console.log('FetchResponseTypePlugin onSuccess json', json);
        return this.toAdapterResponse(json, response, config);
      case 'arraybuffer':
      ...
      console.error('FetchResponseTypePlugin onSuccess error', error);
      throw error;
    }
  }
