...
describe('RouterLoader', () => {
  ...
  describe('toRoute - string element (componentMaps lookup)', () => {
    it('should resolve string element from componentMaps', () => {
      ...
      // Component from componentMaps should be returned directly (not wrapped in lazy)
      const renderCall = mockRender.mock.calls[0][0];
      expect(renderCall.path).toBe('/404');
      // The element should be the NotFound component, not a lazy wrapper
      expect(renderCall.element).toBe(NotFound);
    });
  ...
  describe('toRoute - ReactNode element', () => {
    it('should handle ReactNode element directly', () => {
      ...
      const reactNode = <div>Direct ReactNode</div>;
      const routeConfig: RouteConfigValue = {
        path: '/direct',
        element: reactNode
      };
