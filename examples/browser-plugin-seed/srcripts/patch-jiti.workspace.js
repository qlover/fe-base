...
/**
 * Find pnpm workspace root by walking up for pnpm-workspace.yaml
 */
 ...
 * In pnpm workspace, searches both current package and workspace root node_modules.
 */
    ...
    if (fileExists(pnpmPath)) {
      readDir(pnpmPath).forEach((entry) => {
        if (entry.startsWith('jiti@')) {
          ...
          addFiles(findJitiFiles(packagePath));
        }
        if (
          ...
          const packagePath = path.join(pnpmPath, entry, 'node_modules');
          addFiles(findOxideFiles(packagePath));
        }
      });
    }

    addFiles(findJitiFiles(nodeModules));
    addFiles(findOxideFiles(nodeModules));
  }
