 ...
 * @returns ClearableSignal that aborts when any input signal aborts
 *
 * @example Basic usage
 * ```typescript
...
export function anySignal(
  ...
  if (typeof AbortSignalAny === 'function') {
    ...
    if (validSignals.length > 0) {
      ...
      // Note: Provide clear() for API consistency (no-op for native implementation)
      combinedSignal.clear = () => {};
      return combinedSignal;
    }
  }

  // Note: Fallback to any-signal library for Node.js < 20
