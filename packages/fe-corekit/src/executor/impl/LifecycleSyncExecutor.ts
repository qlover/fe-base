...
import {
  runPluginsHookSync,
  runPluginsHooksSync
} from '../utils/pluginHookSync';

/**
 * Synchronous lifecycle executor implementation
 *
 * Core Concept:
 * A fully synchronous executor that provides complete lifecycle management through
 * plugin hooks without Promise overhead. All operations are synchronous, ensuring
 * predictable execution order and no async context switching.
 *
 ...
 * - Simpler codebase: Reduced complexity compared to mixed sync/async approach
...
> extends BasePluginExecutor<Ctx, Plugin> {
   ...
   * @returns Result of the hook function execution, or undefined
   *
   * @example
   * ```typescript
   ...
   * ```
   *
   * @see runPluginsHookSync - The utility function that performs the actual execution
   */
   ...
   * @returns Result of the last executed hook function, or undefined
   *
   ...
   * ```
   *
   * @see runPluginsHooksSync - The utility function that performs the actual execution
  ...
  protected runExec<Result, Params>(
    ...
    if (!context.hooksRuntimes.times) {
      ...
      if (hookReturnValue === undefined) {
        ...
        // Plugin returned a direct value
  ...
  protected run<Result, Params>(
    ...
    try {
      this.handler(context, actualTask);

      return context.returnValue as Result;
    } catch (error) {
      throw this.handlerCatch(context, error);
    } finally {
      this.handlerFinally(context);
    }
  }
  ...
  protected handler<Result, Params>(
    ...
    this.runHooks(this.plugins, this.getAfterHooks(), context);
   ...
   * @returns ExecutorError to be thrown
   *
  ...
  protected handlerCatch(
    ...
    this.runHook(this.plugins, this.getErrorHook(), context);
    ...
    return new ExecutorError(EXECUTOR_SYNC_ERROR, context.error);
  }
   ...
   * ```
   */
  protected handlerFinally(context: ExecutorContextImpl<unknown>): void {
