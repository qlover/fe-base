...
import { SyncLifecyclePluginInterface } from '../interface/SyncLifecyclePluginInterface';
...
export interface SyncLifecycleExecutorConfig extends PluginExecutorConfig {}
 ...
 * ```typescript
 * const executor = new SyncLifecycleExecutor();
 ...
 * ```typescript
 * class GameEngine {
 *   private executor = new SyncLifecycleExecutor();
 *   
 *   onFrame() {
 ...
 *
 * @category SyncLifecycleExecutor
 */
export class SyncLifecycleExecutor<
  Ctx extends
    ExecutorContextInterface<unknown> = ExecutorContextInterface<unknown>,
  Plugin extends SyncLifecyclePluginInterface<Ctx> = SyncLifecyclePluginInterface<Ctx>
> extends PluginExecutor<Ctx, Plugin>
{
  ...
  protected runHook<Params>(
    ...
    for (let i = 0; i < plugins.length; i++) {
      ...
      const result = hookFn.call(plugin, context as unknown as Ctx, ...args);
  ...
  protected run<Result, Params>(
    ...
    try {
      ...
      const finalError = context.error || new ExecutorError(EXECUTOR_SYNC_ERROR, new Error('Unknown error'));
      throw finalError;
    } finally {
      context.reset();
    }
