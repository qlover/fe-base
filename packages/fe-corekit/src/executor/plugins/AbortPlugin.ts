...
import { AbortPoolConfig, AbortPool, AbortPoolId } from '../../pools/AbortPool';
...
class AbortPluginPool extends AbortPool<AbortPluginConfig> {
  ...
  implements ExecutorPlugin
{
   ...
   * @protected
   */
  protected readonly abortPool: AbortPool<AbortPluginConfig>;
  ...
  public onError({
    ...
    if (AbortPlugin.isAbortError(error)) {
      ...
      const wrapper = this.abortPool.getWrapper(key);
      const reason = wrapper?.controller.signal.reason;
