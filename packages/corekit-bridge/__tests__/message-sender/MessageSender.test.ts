/**
 * @file MessageSender comprehensive test suite
 *
 ...
 * - Utility methods (getMessageStore, getGateway, getDuration)
 */
import {
  ExecutorError,
  Aborter,
...
export function createTestSender(
  ...
  const config: MessageSenderConfig<TestMessage> = {
    gateway: testGateway
  };
...
describe('MessageSender', () => {
  ...
  describe('Edge cases and special scenarios', () => {
    ...
    describe('special scenarios', () => {
      ...
      it('should handle message object containing Symbol property', async () => {
        ...
        expect(result.status).toBe(MessageStatus.SENT);
        expect(
          (result as unknown as { [key: symbol]: string })[symbolKey]
        ).toBe('symbol value');
      });
      ...
      it('should handle message with boolean fields', async () => {
        ...
        expect(
          (result as unknown as { isImportant: boolean }).isImportant
        ).toBe(true);
  ...
  describe('Plugin error handling and throwIfError combination', () => {
    let service: MessageSender<TestMessage>;
    let store: MessagesStore<TestMessage>;
    let mockGateway: MessageGetwayInterface;
