...
describe('AsyncStateAction', () => {
  let store: AsyncStore<TestUser, string>;
  let mockStorage: MockStorage<string>;

  beforeEach(() => {
    mockStorage = new MockStorage();
    store = new AsyncStore<TestUser, string>({
      storage: mockStorage,
...
describe('AsyncStoreInterface', () => {
  ...
  describe('restore', () => {
    it('should return null when storage is not configured', () => {
      const storeWithoutStorage = new AsyncStore<TestUser, string>({
        storage: null,
        defaultState: () => null
      });
    ...
    it('should return null when storageKey is not configured', () => {
      const storeWithoutKey = new AsyncStore<TestUser, string>({
        storage: mockStorage,
        storageKey: null,
        defaultState: () => null
      });
    ...
    it('should return null when storage getItem throws error', () => {
      ...
      const errorStore = new AsyncStore<TestUser, string>({
...
describe('AsyncStore subclass implementation', () => {
   ...
   */
  class ExtendedAsyncStore<T, Key, Opt = unknown> extends AsyncStore<
    T,
    Key,
    Opt
  > {
    ...
    override start(result?: T | undefined): void {
      this.setCustomProperty('started');
      super.start(result);
    }

    override success(result: T): void {
      this.setCustomProperty('success');
      super.success(result);
    }

    override failed(error: unknown, result?: T | undefined): void {
      this.setCustomProperty('failed');
      super.failed(error, result);
    }

    override stopped(error?: unknown, result?: T | undefined): void {
  ...
  let store: ExtendedAsyncStore<TestUser, string>;
  let mockStorage: MockStorage<string>;

  beforeEach(() => {
    mockStorage = new MockStorage();
    store = new ExtendedAsyncStore<TestUser, string>({
      storage: mockStorage,
...
describe('AsyncStore extension patterns', () => {
  ...
  class ExpiringAsyncStore<T, Key, Opt = unknown> extends AsyncStore<
    T,
    Key,
    Opt
  > {
    ...
    constructor(expirationKey: Key, options?: AsyncStoreOptions<T, Key, Opt>) {
      super(options);
    ...
    override restore<R = T | AsyncStoreStateInterface<T>>(): R | null {
      ...
      try {
        const stored = this.storage.getItem(
          this.storageKey
        ) as ExpiringAsyncStoreState<T> | null;
        if (stored) {
          // Check expiration
          if (stored.expires) {
            const expiresAt =
    ...
    setExpiration(expiresInMs: number): void {
      ...
      this.updateState({
        expires
      } as Partial<ExpiringAsyncStoreState<T>>);
    }
  }

  let store: ExpiringAsyncStore<TestUser, string>;
  let mockStorage: MockStorage<string>;

  beforeEach(() => {
    mockStorage = new MockStorage();
    store = new ExpiringAsyncStore<TestUser, string>('expiration-key', {
      storage: mockStorage,
  ...
  describe('complex extension scenarios', () => {
     ...
     */
    class RetryableAsyncStore<T, Key, Opt = unknown> extends AsyncStore<
      T,
      Key,
      Opt
    > {
      ...
      constructor(
        maxRetries: number,
        options?: AsyncStoreOptions<T, Key, Opt>
      ) {
        super(options);
       ...
       * Retry the operation
       */
      async retry(operation: () => Promise<T>): Promise<void> {
    ...
    it('should support retry logic extension', async () => {
      const retryStore = new RetryableAsyncStore<TestUser, string>(3, {
        storage: null,
        defaultState: () => null
      });
    ...
    it('should stop retrying after max retries', async () => {
      const retryStore = new RetryableAsyncStore<TestUser, string>(2, {
        storage: null,
        defaultState: () => null
      });
  ...
  describe('multiple inheritance levels', () => {
     ...
     * Base extended store
     */
    class BaseExtendedStore<T, Key, Opt = unknown> extends AsyncStore<
      T,
      Key,
      Opt
    > {
     ...
     */
    class FurtherExtendedStore<T, Key, Opt = unknown> extends BaseExtendedStore<
      T,
      Key,
      Opt
    > {
