...
class MockGateway {
  async getUser(_params: { id: number }): Promise<TestUser> {
    return {
      id: 1,
      name: 'John Doe',
...
describe('GatewayBasePlguin', () => {
  ...
  let mockStore: AsyncStore<TestUser, string>;
  let mockLogger: MockLogger;
  let mockGateway: MockGateway;

  beforeEach(() => {
    plugin = new GatewayBasePlguin<unknown, TestUser, MockGateway>();
    mockStore = new AsyncStore<TestUser, string>();
  ...
  describe('onBefore', () => {
    it('should call store.start() when store is provided', async () => {
      ...
      const options: GatewayExecutorOptions<unknown, TestUser, MockGateway> = {
        actionName: 'getUser',
    ...
    it('should not throw when store is null', async () => {
      const options: GatewayExecutorOptions<unknown, TestUser, MockGateway> = {
        ...
        serviceName: 'TestService',
        params: {},
        store: null as unknown as AsyncStore<TestUser, string>,
        gateway: mockGateway,
        logger: mockLogger
      };
