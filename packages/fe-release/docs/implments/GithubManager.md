...
| Name        | Type                 | Optional | Default | Since | Deprecated | Description                     |
| ----------- | -------------------- | -------- | ------- | ----- | ---------- | ------------------------------- |
| `workspace` | `WorkspaceInterface` | ❌       | -       | -     | -          | Workspace to create release for |
...
#### `getPullRequestsForCommit` (Method)

**Type:** `(commitSha: string, commitMessage: string) => Promise<Object[]>`
...
##### `getPullRequestsForCommit` (CallSignature)
...
Promise resolving to associated pull requests
...
Error if token is missing, invalid, or lacks repository access
...
### `PullRequestsForCommit` (TypeAlias)

**Type:** `type PullRequestsForCommit`
