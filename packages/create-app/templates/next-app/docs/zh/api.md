...
// 统一响应类型
type AppApiResult<T = unknown> = AppApiSuccessInterface<T> | AppApiErrorInterface;
```
...
export class AppUserApiBootstrap implements BootstrapExecutorPlugin {
  onBefore({ parameters: { ioc } }: BootstrapContext): void {
    ...
    appUserApi.usePlugin(new RequestCommonPlugin({
...
export async function GET(req: NextRequest) {
  ...
  if (result instanceof ExecutorError) {
    return NextResponse.json(
      new AppErrorApi(result.id, result.message),
      { status: 400 }
    );
  }
